...
type FeatureMode = "on" | "variation_1" | "variation_2" | "off";
...
const FEATURE_FLAG_CONFIG: FeatureFlagConfig = {
 ...
 */
async function getFeatureMode(): Promise<FeatureMode> {
  ...
  const envMode = process.env.CONFLUENCE_FEATURE_MODE as FeatureMode;
  return envMode || "on";
}
 ...
 */
function processMarkdownTableEnhanced(
  lines: string[],
  startIndex: number,
  mode: FeatureMode
  ...
  if (headers.length > 0) {
    const headerTag =
      mode === "variation_1" || mode === "variation_2" ? "thead" : "tbody";
 ...
 */
async function convertMarkdownToConfluenceStorageEnhanced(
  markdown: string
): Promise<string> {
  ...
  if (featureMode !== "off") {
    htmlLines.push(
      `<!-- Generated with feature flag: ${FEATURE_FLAG_CONFIG.flagKey}, mode: ${featureMode} -->`
    );
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith("- ")) {
      ...
      const listItemAttributes =
        featureMode === "variation_2" ? ' class="advanced-list-item"' : "";
    ...
    if (line.includes("|") && !line.match(/^\s*\|?\s*-+\s*\|/)) {
      ...
      if (isTableHeader) {
        ...
        const paragraphAttributes =
          featureMode === "variation_2" ? ' class="table-like-text"' : "";
...
export async function readConfluencePage(
  ...
  if (!pageId && !(spaceKey && title)) {
    throw new Error(
      "Either pageId or both spaceKey and title are required to read a Confluence page"
    );
  }
      ...
      if (error.status === 404) {
        throw new Error(
          `Confluence page with ${identifier} not found. This could mean: 1) The page ${
            ...
            pageId ? "page ID" : "page title and space key"
...
export async function createConfluencePage(
  ...
  try {
    ...
    if (error instanceof ConfluenceClientError) {
      ...
      throw new Error(
        `Failed to create Confluence page "${title}" in space "${spaceKey}": ${error.message}`
      );
    }
    throw new Error(
